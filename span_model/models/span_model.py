...
class TagEmbedder(torch.nn.Module):
    def __init__(self,num_width_embeddings,
                dim=256, vocab_size=8):
        super().__init__()
        self.vocab_size = vocab_size
        self.dim = dim
        self.embedder = torch.nn.Embedding(self.vocab_size, self.dim, padding_idx=0)
        self.tag_dict = {
            'RB':1,'RBR':1,'RBS':1,'VB':2,'VBD':2,'VBG':2,'VBN':2,'VBP':2,'VBZ':2,
            'DT':3,'IN':4,'JJ':5,'JJR':5,'JJS':5,'NN':6,'NNS':6,'NNP':6,'NNPS':6,
        }
    def forward(self, text,spans,span_embeddings) -> torch.Tensor:
        ...
        cur_tag_tensor = cur_tag_tensor.to(device)
        ...
        new_span_embeddings = torch.cat((span_embeddings,tag_embeddings),-1)
...
class SpanModel(Model):
    ...
    def forward(
        ...
        if self.use_tag_embeds:
            span_embeddings = self.tag_embedder(metadata.sentences[0].text,spans,span_embeddings)
        span_embeds_b = self.text_to_span_embeds(text_embeds_b, **kwargs)
        if self.use_tag_embeds:
            span_embeds_b = self.tag_embedder(metadata.sentences[0].text, spans, span_embeds_b)
