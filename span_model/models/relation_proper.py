...
class ProperRelationExtractor(Model):
    ...
    def forward(
        ...
        raw_sentences = metadata.sentences[0].text
        pruned_o: PruneOutput = self._prune_spans(spans, span_mask, span_embeddings, sentence_lengths, "opinion", raw_sentences)
        pruned_t: PruneOutput = self._prune_spans(spans, span_mask, span_embeddings, sentence_lengths, "target", raw_sentences)

        if span_embeddings.requires_grad == True:
        ...
        #matrix_filter, matrix_dist = None, None
        relation_scores = self._compute_relation_scores(pruned_o, pruned_t,matrix_filter,matrix_dist)
    ...
    def _prune_spans(self, spans, span_mask, span_embeddings, sentence_lengths, name: str,raw_sentences) -> PruneOutput:
        if not self.use_pruning:
        ...
        outputs = pruner(span_embeddings, span_mask, num_spans_to_keep,raw_sentences =raw_sentences,spans=spans,name=name)
        (
            top_span_embeddings,
            top_span_mask,
            top_span_indices,
    ...
    def _make_pair_features(self, a: torch.Tensor, b: torch.Tensor, matrix_dist) -> torch.Tensor:
    ...
    def _compute_span_pair_embeddings(self, a: torch.Tensor, b: torch.Tensor, matrix_dist) -> torch.Tensor:
        c = self._make_pair_features(a, b, matrix_dist)
        if self.use_bi_affine_classifier:
            c = self._bi_affine_classifier(a, b)
        return c

    def _compute_relation_scores(self, pruned_a: PruneOutput, pruned_b: PruneOutput,matrix_filter,matrix_dist):
        if self.span_length_loss_weight_gamma != 0:
        ...
        for i in range(0, num_a, chunk_size):
            ...
            embeds = self._compute_span_pair_embeddings(a, b,matrix_dist)
