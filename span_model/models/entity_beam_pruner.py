...
class Pruner(torch.nn.Module):
    ...
    def forward(
        ...
        #  这个函数是通过修改分数的方式进行tag_pruned
        # scores = self.tag_modeify_score_pruned(scores,raw_sentences,spans,name)

        # Shape: (batch_size, max_num_items_to_keep, 1)
        _, top_indices = scores.topk(max_items_to_keep, 1)

        #  这个函数是通过减少数量的方式进行tag_pruned
        # process_num_items_to_keep,process_max_items_to_keep,process_top_indices = self.tag_dec_num_pruned(top_indices,raw_sentences,spans,name)
        # if process_num_items_to_keep is not None:
        #     num_items_to_keep = process_num_items_to_keep
        #     max_items_to_keep = process_max_items_to_keep
        #     top_indices = process_top_indices
