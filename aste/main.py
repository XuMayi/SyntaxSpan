# 去除shell.run和Fire新加的部分
import os
import sys
sys.path.append(os.getcwd()+"/aste")
# 去除shell.run和Fire新加的部分
import json
import shutil
import time
...
class SpanModelTrainer(BaseModel):
    ...
    def train(self, overwrite=True):
        ...
        from span_model.data.dataset_readers import span_model  # 导入模型
        from allennlp.commands.train import train_model_from_file
        # Instead of this python code, you would typically just call
        # allennlp train [config_file] -s [serialization_dir]
        train_model_from_file(
            path_config, self.model_path.parent, file_friendly_logging=True, force=True
        )
        # 去除shell.run和Fire新加的部分
    ...
    def predict(self, data_split: SplitEnum) -> Path:
        ...
        p = Namespace(archive_file=self.model_path,
                      input_file=self.get_processed_data_path(data_split),
                      predictor="span_model", use_dataset_reader="span_model", output_file=path
                      , cuda_device=self.train_kwargs["trainer__cuda_device"],
                      silent="", file_friendly_logging=False, weights_file=None, overrides=''
                      , dataset_reader_choice='validation', batch_size=1,predictor_args='')
        from span_model.predictors import span_model
        from span_model.data.dataset_readers import span_model
    ...
    **kwargs,
    ...
    for i, p in enumerate(paths):
        ...
        for k, v in scores.items():
            row = dict(name=p.stem, k=k, Precision=v.precision*100,Recall=v.recall*100,FScore=v.fscore*100, duration=duration)
